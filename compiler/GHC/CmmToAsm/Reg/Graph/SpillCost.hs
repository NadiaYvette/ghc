 ...
 where
        ...
        countLIs scale rsLiveEntry (LiveInstr instr (Just live) : lis)
         = do
                ...
                mapM_ (incUses scale) $ nonDetEltsUniqSet $ mkUniqSet $ mapMaybe (takeVirtualReg . regWithFormat_reg) read
                mapM_ (incDefs scale) $ nonDetEltsUniqSet $ mkUniqSet $ mapMaybe (takeVirtualReg . regWithFormat_reg) written
